...
export const clearActiveVaultKey = () => { activeVaultKey = null; };
...
export const getVaultData = async () => { 
  try { 
    ...
    const isDecoyInt = mode === 'LIMITED' ? 1 : 0; 
    ...
    if (!hasValidDbData) {
        const legacyKey = mode === 'LIMITED' ? 'DECOY_VAULT_DATA' : 'VAULT_DATA';
  ...
  try { 
    ...
    if (db) {
        db.withTransactionSync(() => {
          const keepIds = dataArray.map(item => `'${item.id}'`).join(',');
          try {
